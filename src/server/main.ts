import { Cardtype, Color, PrismaClient } from "@prisma/client";
import { z } from 'zod'
...
function init(): void {
  ...
  prisma.utilities.createMany({
    data: [
      { id: 12, name: 'Electric Company', rent: [4, 10] },
      { id: 28, name: 'Water Works', rent: [4, 10] },
    ]
  })

  prisma.drawable.createMany({
    data: [
  ...
  prisma.property.createMany({
    data: [
      ...
      { id: 12, name: 'States Avenue', color: Color.PINK, price: 140, rent: [10, 20, 50, 150, 450, 625, 725] },
      ...
      { id: 17, name: 'Tennessee Avenue', color: Color.ORANGE, price: 180, rent: [14, 28, 70, 200, 550, 750, 950] },
      ...
      { id: 22, name: 'Indiana Avenue', color: Color.RED, price: 220, rent: [18, 36, 90, 250, 700, 875, 1050] },
      { id: 22, name: 'Illinois Avenue', color: Color.RED, price: 240, rent: [20, 40, 100, 300, 750, 900, 1100] },
...
init() // INFO: Creates all the Database boilerplate
...
app.get("/api/property", async (req, res) => {
  try {
    ...
    const propNameValid = querySchema.parse(propName)
...
app.get("/api/railroad", async (req, res) => {
  try {
    ...
    res.send({ status: 200, data: railroad })
  } catch {
    res.send({ status: 404, data: {} })
  }
});
...
app.get("/api/utility", async (req, res) => {
  try {
    ...
    res.send({ status: 200, data: utilities })
  } catch {
    res.send({ status: 404, data: {} })
  }
});
...
app.get("/api/spot/:id", async (req, res) => {
  try {
    ...
    const id = idSchema.parse(idUnsafe)
    let data
